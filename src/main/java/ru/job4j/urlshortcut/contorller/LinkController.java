...
import ru.job4j.urlshortcut.service.PrincipalService;
...
public class LinkController {
    ...
    public Map<String, String> convert(@Valid @RequestBody Link link,
        ...
        Link registeredLink = simpleLinkService
                .save(link, simplePrincipalService.getSiteLogin(), length);
        return Map.of(
    ...
    public List<Map<String, String>> statistic() {
        List<Link> links = simpleLinkService
                .findAllBySite(simplePrincipalService.getSiteLogin());
        return links.stream()
                .map(x -> Map.of(
                        "url", x.getUrl(),
